...
- Upon selecting a study, we will store(command yet to be implemented + tested) any study currently held locally (if there is any study to store) and delete the local copy(if storing doesn't already remove it)
- When the local location of our dicom files is empty, we will retrieve(yet to be implemented + tested) the chosen study
- After loading this study, we then must display it (this will involve changing the current implementation of file loading only slightly)
...
store (as the dataset is currently empty)  
retrieve  
search  
